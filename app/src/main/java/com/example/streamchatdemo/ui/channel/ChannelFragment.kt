...
class ChannelFragment : Fragment() {
    ...
    private fun setupUser() {
        ...
        if (client.getCurrentUser() == null) {
            user = if (args.chatUser.image.isNotEmpty()) {
                User(
                    id = args.chatUser.username,
                    extraData = mutableMapOf(
                        "name" to args.chatUser.firstName,
                        "image" to args.chatUser.image,
                        "type" to args.chatUser.avatar
                    )
                )
            } else {
                User(
            ...
            client.connectUser(
                ...
                if (result.isSuccess) {
                    Log.d("ChannelFragment", result.toString())
                } else {
    ...
    private fun logout() {
        ...
        builder.setPositiveButton("Yes") { _, _ ->
            ...
            if(args.chatUser.avatar == "doctor") {
                startActivity(Intent(activity, doctor::class.java))
            }else{
                startActivity(Intent(activity, patients::class.java))
            }
